...
Что делает:
  1. Прогоняет модель через изображения, считает ошибку кастомным лоссом (DiceBCELoss).
  2. Обновляет веса, измеряет метрику AIC на валидации.
  3. Сохраняет лучшие веса (best.pth), если AIC улучшился.
  4. Сохраняет чекпоинт (last.pth) для возобновления обучения.
...
from src.metrics import calculate_aic
...
# ЛОГИРОВАНИЕ
# ============================================================
def log_metrics(epoch, train_loss, val_loss, val_aic, log_path='results/metrics.csv'):
    ...
    with open(log_path, 'a', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['epoch', 'train_loss', 'val_loss', 'val_aic'])
        if not file_exists:
            writer.writeheader()
        writer.writerow({
            'epoch': epoch,
...
# ============================================================
# ВАЛИДАЦИЯ: ОДНА ЭПОХА (инкрементальный расчёт AIC)
# ============================================================
@torch.inference_mode()
def eval_epoch(model, loader, criterion, device, threshold: float = 0.5):
    """Одна эпоха валидации с инкрементальным расчётом AIC."""
    if len(loader) == 0:
        return 0.0, 0.0
    ...
    for images, masks in pbar:
        ...
        # Инкрементальный расчёт метрик по батчу
        for i in range(preds.shape[0]):
            gt, pred = targets[i], preds[i]
            gt_sum, pred_sum = gt.sum(), pred.sum()

            if gt_sum > 0:  # Positive
                n_pos += 1
                intersection = (pred * gt).sum()
                total_dice += 2.0 * intersection / (pred_sum + gt_sum + 1e-6)
            else:  # Negative
                n_neg += 1
                if pred_sum >= 0.01 * pred.size:
                    n_false_alarm += 1

        pbar.set_postfix({"loss": f"{loss_val:.4f}"})

    # Защита: если в валидации нет positive-примеров
    if n_pos == 0:
        print("⚠️ WARNING: В валидации нет positive-примеров. AIC не может быть посчитан.")
...
def resume_training(model, optimizer, scheduler, save_dir='checkpoints'):
    """Загружает состояние из last.pth для возобновления обучения."""
...
def train(model, train_loader, val_loader, epochs=10, lr=1e-3,
    ...
    # Попытка возобновить обучение из last.pth
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print(f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val AIC: {val_aic:.4f}")
        log_metrics(epoch, train_loss, val_loss, val_aic)

        # Шаг скедулера (ориентируемся на максимизацию AIC)
        scheduler.step(val_aic)

        # Сначала обновляем best_aic и сохраняем best.pth
        if np.isnan(val_aic) or np.isinf(val_aic):
            print(f"⚠️ WARNING: val_aic = {val_aic}, пропускаем сохранение лучшей модели")
        elif val_aic > best_aic:
            best_aic = val_aic
            best_path = os.path.join(save_dir, 'best.pth')
            torch.save(model.state_dict(), best_path)
            print(f"🌟 Новый лучший AIC: {best_aic:.4f}! Модель сохранена.")

        # Затем сохраняем актуальный last.pth (с обновлённым best_aic)
        last_path = os.path.join(save_dir, 'last.pth')
        torch.save({
            'epoch': epoch,
